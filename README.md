...
I would also not look to this project for inspiration on how to do agents. Agents have evolved overtime and the methodology today (as of 2026) would probably allow for more tasks to be compressed within a single call. This project makes a separate LLM call with a different prompt anytime the agent needs to pick a search method, analyze responses, etc. while today's agents can do this fully autonomously with no problem. In hindsight, this may be due to the fact that reasoning was still fairly "new" and companies were still in the process of scaling up RL. Long horizon tasks were not as efficient, and context rot was a real issue. At the time, Gemini 2.5 was just released and considered SOTA.
